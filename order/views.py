...
from util.utils import login_decorator
...
class OrderView(View):
    ...
    def get(self, request):
        user_id = request.user.id
        user_products = Order.objects.filter(user_id=user_id, order_status_id=1).prefetch_related('orderitem_set',
                                                                                                  'orderitem_set__product').all().order_by(
            '-id')
